...
public class Base64Test {
	String[] input = {"Ä", "This is plain ASCII", "AZöby7ß", "abcö", "Polyfon " +
			"zwitschernd " + "aßen Mäxchens Vögel Rüben, Joghurt und Quark"};
	String[] expected = {"w4Q=", "VGhpcyBpcyBwbGFpbiBBU0NJSQ==", "QVrDtmJ5N8Of",
			"YWJjw7Y" + "=",
			"UG9seWZvbiB6d2l0c2NoZXJuZCBhw59lbiBNw6R4Y2hlbnMgVsO2Z2VsIFLDvGJlbiwgSm9naHVy" +
					"\ndCB1bmQgUXVhcms="};
	String jwt = "eyJhbGciOiJSUzI1NiIsImtpZCI6ImRqYXJqb1VzZXIiLCJ0eXAiOiJKV1QifQ." +
			...
			"d6Mfr2L2myIbkxgeqRIhzivmPy63D1-iF8D8WvZSwE5QWoqpHU8XSzJTe2Y-xL451K" + "v2ko" +
			"-jDG1PWhuL8jxUG5e-RcDQ";
	...
	private void checkJwt() {
		...
		String header = new String( Base64.decoder()
				.decode( parts[0] ) );
		String payload = new String( Base64.decoder()
				.decode( parts[1] ) );
		String signature = new String( Base64.decoder()
				.decode( parts[2] ) );
		try {
	...
	public void testDecoding() {
		...
		for ( int i = 0; i < expected.length; i++ ) {
			result = new String( Base64.decoder()
					.decode( expected[i] ), StandardCharsets.UTF_8 );
			assertEquals( input[i], result, "Checking index " + i );
		}
	}
	...
	public void testEncoding() {
		...
		for ( int i = 0; i < input.length; i++ ) {
			result = Base64.encoder()
					.withStandard()
					.withLineBreaks()
					.encode( input[i] );
			assertEquals( expected[i], result, "Checking index " + i );
		}
	}
