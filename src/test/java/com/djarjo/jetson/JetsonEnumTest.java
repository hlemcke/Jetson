...
public class JetsonEnumTest {
  ...
  void testDecodeEnumWithAccessor() throws ParseException, IllegalAccessException {
    //--- given
    List<String> inputs = List.of( "code2", "ENUM_VALUE_3", "x", "X" );
    List<SomeEnum> expected = List.of( SomeEnum.ENUM_VALUE_2, SomeEnum.ENUM_VALUE_3,
        SomeEnum.LOWER, SomeEnum.UPPER );

    //--- when / then
    for ( int i = 0; i < inputs.size(); i++ ) {
      String input = String.format( "{\"someEnum\":\"%s\"}", inputs.get( i ) );
      PojoCode pojo = new PojoCode();
      Jetson.decodeIntoObject( input, pojo );
