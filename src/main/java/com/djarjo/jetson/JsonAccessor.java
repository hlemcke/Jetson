...
public record JsonAccessor(Class<?> clazz, JsonConfig config, Field field, Method getter,
  ...
  public Object getValue( Object bean ) {
    try {
      Object value = isClass() ? bean : isField() ? field.get( bean ) : getter.invoke( bean,
          (Object[]) null );
      if ( hasConverter() ) {
   ...
   */
  public boolean mayEncode( Object value, boolean skipEmpty, boolean skipNull ) {
    if ( isClass() ) return true;

    return switch ( config.encode() ) {
      case ALWAYS -> true;
      case INHERITED ->
          (value == null) ? !skipNull : BeanHelper.isEmpty( value ) ? !skipEmpty : true;
      case NEVER -> false;
      case ONLY_IF_NOT_EMPTY -> BeanHelper.isNotEmpty( value );
      case ONLY_IF_NOT_NULL -> value != null;
    };
  }
