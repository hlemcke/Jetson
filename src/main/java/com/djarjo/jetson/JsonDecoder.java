...
public class JsonDecoder {
  ...
  private final static Object NO_CONVERTER = new Object();
  ...
  public Object decodeIntoObject( String json, Object target )
    ...
    if ( target != null ) {
      Object result = _handleClassAnnotation( target.getClass(), target );
      if ( result != NO_CONVERTER ) return result;
    }
   ...
   */
  public Object decodeIntoObjectWithGeneric( String json, Object target,
      Class<?> genericType )
      throws ParseException, IllegalAccessException {
  ...
  private Object _decodeJsonValue( Object target, Type targetType )
    ...
    Token token = _tokenizer.getToken();
    if ( (token.symbol == Symbol.WORD) && "null".equalsIgnoreCase( token.value ) ) {
      return null;
    }
    ...
    if ( (rawClass != null) && rawClass.isAnnotationPresent( Json.class ) ) {
      Object result = _handleClassAnnotation( rawClass, target );
      if ( result != NO_CONVERTER ) return result;
      if ( target == null ) {
        if ( rawClass.isRecord() ) {
          throw new ParseException(
              "Record " + rawClass.getSimpleName() + " requires a 'static fromJson(String)' method " +
                  "or a JsonConverter because it lacks a no-args constructor.",
              _tokenizer.getPosition()
          );
        }
        target = ReflectionHelper.createInstance( rawClass );
      }
    }
   ...
   * <p>
   * If {@literal @Json} specifies a converter then the current token value (which must
   * be a string) will be given to converters decode method.
   * </p><p>
   * If the class contains a method named {@code fromJson(String)} then the current
   * token value (which must be a string) will be given to that method.
   * </p>
   * If the class is not annotated with {@literal @Json} then {@link #NO_CONVERTER} will
   * be returned.
   *
   * @param target current bean
   * @return value from converter or from {@code fromJson(String)} or {@link #NO_CONVERTER}
   */
  @SuppressWarnings("rawtypes")
  private Object _handleClassAnnotation( Class<?> clazz, Object target ) {
    if ( clazz == null && target != null ) clazz = target.getClass();
    if ( clazz == null ) return NO_CONVERTER;

    // --- Check if class is annotated with @Json
    Json classAnno = clazz.getAnnotation( Json.class );
    if ( classAnno == null ) return NO_CONVERTER;
    ...
    //--- try static method "fromJson()" if it exists in POJO class. Critical for records
    Method fromJson = ReflectionHelper.findMethod( clazz, METHOD_FROM_JSON );
    if ( fromJson != null ) {
      try {
        return fromJson.invoke( null, token.value );
      } catch ( IllegalAccessException | InvocationTargetException e ) {
        logger.atWarning().withCause( e )
            .log( "Cannot invoke %s.fromJson( %s )", clazz.getName(), token.value );
      }
    }
    return null;
  }
