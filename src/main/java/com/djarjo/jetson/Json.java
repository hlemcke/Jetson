...
public @interface Json {
  ...
  /**
   * Controls encoding a JSON string. Defaults to {@code EncodeMode.INHERITED}
   *
   * @return mode
   */
  EncodeMode encode() default EncodeMode.INHERITED;
  ...
  enum DecodeMode {
    ...
    /**
     * Decode only if bean property currently is {@code null}
     */
    ONLY_IF_NULL
  }
  ...
  enum EncodeMode {
    ...
    /**
     * Encode only if value is not {@code null} and not empty
     */
    ONLY_IF_NOT_EMPTY,

    /**
     * Enocde only if value is not {@code null}
     */
    ONLY_IF_NOT_NULL
  }
}