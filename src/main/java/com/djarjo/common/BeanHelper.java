...
public class BeanHelper {
  ...
  /**
   * Gets accessors for given class.
   * <p>
   * Fetches all fields. If the field has getter and setter then returned accessor
   * contains the methods else the field.
   * </p>
   *
   * @param cls well ... class
   * @return List of accessors (can be empty) sorted by name of field
   */
  public static List<MemberAccessor> getAccessors( Class<?> cls ) {
    List<MemberAccessor> accessors = new ArrayList<>();
    Field[] fields = cls.getDeclaredFields();
    Method[] methods = cls.getDeclaredMethods();
    for ( Field field : fields ) {
      if ( field.isSynthetic() ) continue;
      Method getter = ReflectionHelper.findGetter( cls, field.getName() );
      Method setter = ReflectionHelper.findSetter( cls, field.getName() );
      accessors.add( new MemberAccessor( field, getter, setter ) );
    }
    accessors.sort( ( a, b ) -> a.getName().compareTo( b.getName() ) );
    return accessors;
  }
