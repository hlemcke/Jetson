...
public class BaseConverter {
  ...
  public static URI toURI( Object value ) {
    if ( value == null ) return null;
    if ( value instanceof URI u ) return u;
    if ( value instanceof String s ) return URI.create( s );
    logger.atWarning().log( "Cannot convert to URI: '%s'", value );
    return null;
  }
  ...
  private static Map<Class<?>, Function<Object, Object>> _getConverters() {
    if ( _converters.isEmpty() ) {
      ...
      _converters.put( URI.class, BaseConverter::toURI );
