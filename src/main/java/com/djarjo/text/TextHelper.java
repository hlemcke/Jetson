...
public class TextHelper {
	...
	/**
	 * The maximum Levenshtein distance by which two words are identical. The
	 * value {@value} is a percentage (1=10%, 2=20%, ...)
	 *
	...
	/**
	 * Replaces tabs with space and condenses multiple spaces into one single
	 * space.<br />
	 * Value for parameter 'option' in {@link #condense(String, int)}.<br />
	 * Multiple options can be added.
	 */
	public final static int CONDENSE_BLANKS = 1;

	/**
	 * Removes all tabs, spaces, CRs and LFs.<br />
	 * Value for parameter 'option' in {@link #condense(String, int)}.<br />
	 * Multiple options can be added.
	 */
	...
	/**
	 * Removes carriage returns (useless Microsoft line break)<br />
	 * Value for parameter 'option' in {@link #condense(String, int)}.<br />
	 * Multiple options can be added.
	 */
	...
	/**
	 * Removes end of line comments starting with "//..." until end of line
	 * <br />
	 * Value for parameter 'option' in {@link #condense(String, int)}.<br />
	 * Multiple options can be added.
	 */
	...
	/**
	 * Removes annotation "@Generated(...)"<br />
	 * Value for parameter 'option' in {@link #condense(String, int)}.<br />
	 * Multiple options can be added.
	 */
	...
	/**
	 * Removes remark lines starting with "# " or "-- "until end of line<br />
	 * Value for parameter 'option' in {@link #condense(String, int)}.<br />
	 * Multiple options can be added.
	 */
	...
	/**
	 * Removes tabulator characters<br />
	 * Value for parameter 'option' in {@link #condense(String, int)}.<br />
	 * Multiple options can be added.
	 */
	...
	/**
	 * Removes spaces and tabs at start and end of each line<br />
	 * Value for parameter 'option' in {@link #condense(String, int)}.<br />
	 * Multiple options can be added.
	 */
	...
	/**
	 * Collapses the text by reducing all occurrences of white space (tab,
	 * blank, cr, lf) to a single blank character.<br />
	 * Value for parameter 'option' in {@link #condense(String, int)}.<br />
	 * Multiple options can be added.
	 */
	...
	/**
	 * Removes all comments<br />
	 * Value for parameter 'option' in {@link #condense(String, int)}. Values
	 * can be added.
	 */
	...
	/**
	 * Removes all comments, blanks and empty lines.<br />
	 * Value for parameter 'option' in {@link #condense(String, int)}
	 */
	...
	public static String computeSoundex( String word ) {
		...
		String soundex = "" + Character.toUpperCase( word.charAt( 0 ) );
		char c;
	...
	public static Map<String, String> getMapFromLdapString(
			String ldapString ) {
		Map<String, String> ldapAsMap = new HashMap<>();
		if ( ldapString != null ) {
			ldapString = ldapString.replace( " ", "" );
			String[] ldapItems = ldapAsMap.toString().split( "," );
			for ( String ldapItem : ldapItems ) {
				String[] parts = ldapItem.split( "=" );
				if ( parts.length == 2 ) {
					ldapAsMap.put( parts[0], parts[1] );
				}
			}
		}
		return ldapAsMap;
	...
	public static String getWord( String str, int offset ) {
		int j = offset;
		while ( j < str.length() ) {
			if ( Character.isLetter( str.charAt( j ) ) == false )
				break;
			j++;
		}
	...
	/**
	 * Checks if {@code digits} does only contain digits. Must contain at least
	 * one digit to return {@code true}
	 *
	 * @param digits
	 *            String to check
	...
	public static boolean isDigitsOnly( String digits ) {
		...
		for ( int i = 0; i < digits.length(); i++ ) {
			if ( Character.isDigit( digits.charAt( i ) ) == false ) {
				return false;
			}
		}
		return true;
	...
	public static boolean isSameByLevenshtein( String word1, String word2 ) {
		...
		if ( distance * 10 <= len * MAX_LEVENSTHEIN_DISTANCE ) {
			return true;
		}
		return false;
	}
	...
	/**
	 * Makes a string from the given object. If the object is a collection
	 ...
	 * @return a new instance of {@code LocalDate} or {@code null}
	 */
	...
	public static LocalDate parseDate( final String text,
		...
		LocalDate date = parser.parseDate( text, pattern );
		return date;
	}
	 ...
	 *
	 * @param text
	 *            A double value in text form
	 * @return Returns the double or <em>null</em> if the text is null or no
	 *         double
	 */
	 ...
	 *
	 * @param text
	 *            A float value in text form
	 * @return Returns the float or <em>null</em> if the text is null or no
	 *         float
	 */
	...
	public static String parseNumber( final String text ) {
		...
		// --- Both separators exist => right one is decimal. Remove others
	...
	public static int skipSpace( String str, int start ) {
		while ( start < str.length() ) {
			if ( Character.isWhitespace( str.charAt( start ) ) == false )
				return start;
			start++;
		}
		return start;
		...
		for ( int i = 0; i < size; i++ ) {
			if ( text.charAt( i ) == '\\' ) {
				...
				if ( backslashIndex == to - 1 ) {
					line += text.substring( from, backslashIndex );
				} else {
