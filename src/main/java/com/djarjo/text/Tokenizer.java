...
public class Tokenizer {
  ...
  private final List<Token> _lookAheadList = new ArrayList<>();
  /**
   * Current options to be used for next token
   */
  private final Set<TokenizerOption> _options = new HashSet<>();
  ...
  /**
   * Advances tokenizer to any one of the given symbols.
   * <p>Returns symbol found
   * with value text from current symbol until found one.
   *
   * @param symbols Any of these symbol stops reading
   * @return token with symbol found and value as text between
   */
  ...
  public String getClipping() {
    ...
    return String.format( "...%s<<<%s...", _text.substring( i, _curPos ),
        _text.substring( _curPos, j ) );
  }

  /**
   * Gets the current column number which is the number of characters since
   * the last line feed.
   *
   ...
   * position. Uses options from constructor.
   */
  private void _parse() {
    _token = new Token( Symbol.EOF );
    _token.position = _curPos;
    if ( _curPos >= _textLength ) {
      return;
    }
    char chr = _nextChar();
    ...
    // --- Token starting with "@"
      ...
      if ( Character.isLetter( _getChar() ) ) {
        ...
        while ( Character.isLetterOrDigit( _getChar() ) ) {
          builder.append( _getChar() );
          _nextChar();
        }
        _token.value = builder.toString();
      }
    }
    // --- Token starting with "*"
    else if ( chr == '*' ) {
  ...
  private void _parseString( char matchingChar ) {
    ...
    while ( true ) {
      char c = _getChar();
      if ( c == 0 ) {
        break; // End of text = string is incomplete
      }
      if ( c == matchingChar ) {
        _nextChar(); // end of string reached
        break;
      }
      if ( c == '\\' ) {
        _nextChar(); // consume backslash
        value.append( _unescapeStringPart() );
      } else {
        value.append( c );
        _nextChar();
      }
  ...
  private char _unescapeStringPart() {
    char esc = _nextChar(); // first char following '\'
    switch ( esc ) {
      case '"':
        return '"';
      case '\\':
      ...
      case 'u': { // uXXXX
        ...
        for ( int i = 0; i < 4; i++ ) {
          ...
          if ( v < 0 ) {
            throw new RuntimeException( "Invalid unicode escape in string" );
          }
          code = (code << 4) | v;
        }
        return (char) code;
