...
public class DateTimeParser {
    ...
    public LocalTime parseTime(String text) {
        ...
        if (numbers.length == 1) {
            ...
            if (text.length() > 6) {
                fraction = _fractionToNanos(text.substring(6));
            }
        }
        // --- Non-compact => use separated values
            ...
            if (numbers.length > 3)
                fraction = _fractionToNanos(numbers[3]);
        }
    ...
    private int _fractionToNanos(String fraction) {
        if (fraction == null || fraction.isEmpty()) {
            return 0;
        }
        // Ensure max 9 digits (nanosecond precision)
        if (fraction.length() > 9) {
            fraction = fraction.substring(0, 9);
        }
        // Right-pad with zeros to reach nanoseconds
        String nanosString = String.format("%-9s", fraction).replace(' ', '0');
        return Integer.parseInt(nanosString);
    }
